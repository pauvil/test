...
public class PrintGvPazymaAction extends Action 
{
		...
		throws DatabaseException, InternalException, PermissionDeniedException, ObjectNotFoundException
	{
		...
		GvPazyma pazyma = (GvPazyma)session.getAttribute("gvPazyma");
		GyvenamojiVieta gvt = pazyma.getGyvenamojiVieta();		
		AtvykimoDeklaracija deklaracija = null;
		try {
			//Deklaracija temp = pazyma.getDeklaracija().getId();
			//deklaracija = (AtvykimoDeklaracija)pazyma.getDeklaracija();
			if(pazyma.getDeklaracija()!= null)
				deklaracija = DeklaracijosDelegator.getInstance(request).getAtvykimoDeklaracija(new Long(pazyma.getDeklaracija().getId()), request);
		}
		catch (Exception cce) { deklaracija = null;}
		...
		List vaikai = (List)session.getAttribute("gvVaikai");
		...
		if ("on".equals(request.getParameter("vaikai"))){
			...
			if (gvt != null && asmuo != null){
				...
				for(ListIterator iter = vaikai.listIterator(); iter.hasNext(); ){
					...
					long vaikoNr = vaikas.getAsmNr();
					Asmuo vaikasTemp = QueryDelegator.getInstance().getAsmuoByAsmNr(request, vaikoNr);
					GyvenamojiVieta vaikoGvt = (GyvenamojiVieta)vaikasTemp.getGyvenamosiosVietos().iterator().next();
					if (vaikoGvt.getGvtDataNuo()== null || "".equals(vaikoGvt.getGvtDataNuo())){
						iter.remove();
					}
				}
			}
		...
		try {
			...
			FileInputStream fr = new FileInputStream(this.getServlet().getServletConfig().getServletContext().getRealPath("") + separator + "files" + separator + "GvPazyma.xml");
			byte b;
			...
			if (gvt!= null)
			{
				Constants.Println(request, "++++++++++++++Gvt NR:" + gvt.getGvtNr());
				Constants.Println(request, "++++++++++++++Gvt Asm NR:" + gvt.getGvtAsmNr());
				if (null != deklaracija && null != deklaracija.getDeklaravimoData()) {
					sdate = sdf.format(deklaracija.getDeklaravimoData());
					Constants.Println(request, "++++++++++++++Deklaracija, kai gvt yra" + sdate);
				}
				
				if (gvt.getGvtDataNuo()!= null) {
					sdate = sdf.format(gvt.getGvtDataNuo());
					Constants.Println(request, "++++++++++++++Gvt, kai gvt yra" + sdate);
				} 				

			} else if (null != deklaracija) {
				if(null != deklaracija.getDeklaravimoData()) {
					sdate = sdf.format(deklaracija.getDeklaravimoData());
					Constants.Println(request, "+++++++++++++++Deklaracija, kai gvt nera" + sdate);
				}
			}
